...
export function KdvCalculator({ onApply }: KdvCalculatorProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full z-20 mt-2 w-72 rounded-xl border border-border bg-card p-4 shadow-lg dark:border-border dark:bg-card">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-semibold text-foreground dark:text-foreground">KDV Hesaplayıcı</span>
            <button
              type="button"
          ...
          <div className="flex gap-1 p-1 bg-secondary dark:bg-secondary rounded-lg mb-3 text-xs">
            <button
              ...
              className={`flex-1 py-1.5 rounded-md transition-colors ${mode === 'net-to-gross' ? 'bg-card dark:bg-card shadow-sm font-medium text-foreground dark:text-foreground' : 'text-muted-foreground'}`}
            >
              KDV Hariç → Dahil
            </button>
            <button
              ...
              className={`flex-1 py-1.5 rounded-md transition-colors ${mode === 'gross-to-net' ? 'bg-card dark:bg-card shadow-sm font-medium text-foreground dark:text-foreground' : 'text-muted-foreground'}`}
            >
              KDV Dahil → Hariç
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-lg bg-muted dark:bg-secondary/40 p-3 text-xs space-y-1">
              <div className="flex justify-between text-muted-foreground">
                <span>KDV Hariç</span>
                <span className="font-medium text-foreground dark:text-foreground">{formatTRY(result.net)}</span>
              </div>
              <div className="flex justify-between text-muted-foreground">
                <span>KDV Tutarı (%{rate})</span>
                <span className="font-medium text-foreground dark:text-foreground">{formatTRY(result.kdvAmount)}</span>
              </div>
              <div className="flex justify-between border-t border-border pt-1 mt-1 dark:border-border">
                <span className="font-semibold text-foreground dark:text-foreground">KDV Dahil Toplam</span>
                <span className="font-bold text-foreground dark:text-foreground">{formatTRY(result.gross)}</span>
              </div>
            </div>
            ...
            <button
              ...
              disabled={parsed <= 0}
              className="w-full rounded-lg bg-primary hover:bg-primary/90 disabled:opacity-50 text-white text-xs font-medium py-2 transition-colors dark:bg-secondary dark:text-foreground dark:hover:bg-secondary/70"
            >
