...
export function BulkPasteModal({ categories, userId, onClose, onImport }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-3xl shadow-xl border border-slate-100 max-h-[90vh] flex flex-col">
        ...
        {result ? (
          ...
          <>
            <p className="text-sm text-slate-500 mb-2">
              ...
              <span className="font-medium text-slate-700">Tarih, Açıklama, Tutar, Tip (gelir/gider), Kategori (opsiyonel)</span>.
            </p>
            <details className="mb-3 text-xs text-slate-500">
              <summary className="cursor-pointer hover:text-slate-700 select-none">Örnek biçim</summary>
              <pre className="mt-2 bg-slate-50 border border-slate-200 rounded-lg p-3 whitespace-pre-wrap font-mono">
                {EXAMPLE}
              </pre>
            </details>
            ...
            <textarea
              ...
              rows={6}
              className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 font-mono text-sm resize-y"
            />

            {parsedRows.length > 0 && (
              <div className="mt-4 flex-1 min-h-0 flex flex-col">
                <div className="flex items-center gap-4 mb-2 text-sm">
                  <span className="text-slate-600">{parsedRows.length} satır bulundu</span>
                ...
                <div className="border border-slate-200 rounded-lg overflow-auto flex-1 min-h-0">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-slate-50 border-b border-slate-200 sticky top-0">
                      <tr>
                        <th className="p-2 font-medium text-slate-500 w-8"></th>
                        <th className="p-2 font-medium text-slate-500">Tarih</th>
                        <th className="p-2 font-medium text-slate-500">Açıklama</th>
                        <th className="p-2 font-medium text-slate-500">Tutar</th>
                        <th className="p-2 font-medium text-slate-500">Tip</th>
                        <th className="p-2 font-medium text-slate-500">Kategori</th>
                      </tr>
                    </thead>
                    <tbody>
                      {parsedRows.map((row) => (
                        <tr key={row.lineNumber} className="border-b border-slate-100 last:border-0">
                          ...
                          {row.errors.length > 0 ? (
                            ...
                            <>
                              ...
                              <td className="p-2 align-top text-slate-700">{row.description}</td>
                              <td className="p-2 align-top text-slate-700">
                                {new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(
                                  row.amount as number
                                )}
                              </td>
                              <td className="p-2 align-top text-slate-700">{row.type}</td>
                              <td className="p-2 align-top text-slate-700">
                                {row.categoryName ? (
                                  row.categoryId ? (
                                    row.categoryName
